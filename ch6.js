...
// current() returns the current element.  next() returns the next element.

function logFive(sequence) {
    var counter = 0;
    var result = false;
    while ((sequence.current() != null) && (counter < 5)) {
...
//logFive(new ArraySeq([2, 3, 1, 32, 12]));
logFive(new ArraySeq([2, 3, 1, 32, 54, 23, 12]));
//  2, 3, 1, 32, 54      // Each number is actually on a separate line. Also, we're only printing 5,
                         // so the 23 and the 12 do not print.

logFive(new ArraySeq([]));
//     // prints nothing

logFive(new ArraySeq([456]));
//  456
...
/*
// The book's solution to this last exercise (6.3):
